/* SubAdmin modal */
const createSubAdminModal = document.querySelector('.createSubAdminModal');
const updateSubAdminModal = document.querySelector('.updateSubAdminModal');
function modalCreateSubAdmin() {
    createSubAdminModal.classList.toggle('hidden');
}
function modalUpdateSubAdmin() {
    updateSubAdminModal.classList.toggle('hidden');
}

/* Manager Modal */
const createManagerModal = document.querySelector('.createManagerModal');
const updateManagerModal = document.querySelector('.updateManagerModal');

function modalCreateManager(){
    createManagerModal.classList.toggle('hidden');
}
function modalUpdateManager(){
    updateManagerModal.classList.toggle('hidden');
}